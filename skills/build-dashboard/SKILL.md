...
**If a data source (warehouse, spreadsheet, or export) is available:**
1. Query or read the necessary data
...
Use the Chart.js integration patterns below for each chart type. For guidance on choosing between chart types, see the `data-visualization` skill's chart selection guide.
...
- If a stakeholder only read the KPI row, would they walk away with the right headline, or would they need the detail table to avoid a wrong conclusion?
- Which filter would someone reach for first to challenge this dashboard's story — and does it hold up once applied?
- Is this dashboard a one-time snapshot being presented as if it's live, or is that distinction made clear to the audience?

## Improvement Loop

After a dashboard goes in front of stakeholders, note which KPI or chart prompted the most questions or pushback — that's a signal either the metric needs a guardrail companion or the framing needs to be clearer next time. Reusing a dashboard's structure across business reviews is itself evidence of "Business Outcome Ownership": the same metrics tracked consistently over time, not reinvented each cycle.

...
3. **Gather estimates per item.** For each item, collect the inputs the chosen framework needs (Reach/Impact/Confidence/Effort for RICE; Impact/Confidence/Ease for ICE). Note the source of each estimate (analytics, engineering, a stakeholder guess) — an estimate with no stated source is itself a signal to look at harder in step 4. If two sources give conflicting estimates for the same item, resolve which one is used for scoring per the source-hierarchy rule in Constraints, rather than picking or averaging silently.
...
- **When sources disagree on an item's estimate, resolve the conflict with the repo's source hierarchy — never by averaging or silently picking one.** Two sources giving different numbers for the same item (engineering says 8 weeks of effort, the requesting stakeholder says 2) is the normal case, not an edge case. Use the source hierarchy from `CONTRIBUTING.md` — current evidence/analytics over a working engineering estimate over a stakeholder guess — to decide which number is used for scoring, and note next to that item's score which source won and that a conflict existed. Averaging conflicting numbers, or picking whichever is more convenient, is the same laundering-a-guess failure this skill forbids for missing estimates.

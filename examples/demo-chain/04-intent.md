...
These two changes are stated in the same observable terms `03-opportunity-framing.md`'s Opportunity Statement already uses: it describes the core problem as agents who "re-ask customers questions that were already answered" and a frontline agent who has "stopped trusting the existing handoff mechanism enough to build a personal workaround." Success here is the visible absence of both — fewer repeated questions, and no need for an agent to keep a private workaround because the official mechanism is actually usable. Nothing in the underlying transcripts or research synthesis states a success goal in genuinely vague or unmeasurable terms ("users will love it"-style language); there is no stakeholder wish on record here that needed to be pushed back on and rewritten, so this section is drafted directly from the opportunity statement's own language rather than from a translated quote.
